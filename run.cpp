...
void sampleUsage() {
	...
	tracker.topMarketCaps(4); // return vector with vector.at(0) == "StockA" and vector.at(1) == "StockB"

    // To get the 3rd point, your code must be able to handle ties in market cap
	tracker.registerTicker("IPO1", 100);
	tracker.registerTicker("IPO2", 200);
	tracker.registerTicker("IPO3", 500);
	tracker.updateCurrentPrice("IPO1", 20.0);
	tracker.updateCurrentPrice("IPO2", 10.0);
	tracker.updateCurrentPrice("IPO3", 2.0);
	tracker.getMarketCap("IPO1"); // return 1160.0
	tracker.getMarketCap("IPO2"); // return 1050.0
	tracker.getMarketCap("IPO3");
	tracker.topMarketCaps(2); // top 2 must return IPO1 and IPO2. The order doesn't matter since they are tied
...
void realDataUsage(){
    ...
    ifstream sharesOutstanding;
    ifstream currentPrices;

    sharesOutstanding.open(totalSharesFilename);
    currentPrices.open(pricesFilename);

	// Always check to see if file opening succeeded
	if (!sharesOutstanding.is_open())
	{
    ...
    string lineFromFile;
    getline(sharesOutstanding, lineFromFile); // Strip reference

    // Assumes specific format is followed
    while(getline(sharesOutstanding, lineFromFile)){
        string ticker = lineFromFile.substr(0,lineFromFile.find(","));
        // Market cap in thousands of shares
        int totalShares = std::stoi(lineFromFile.substr(lineFromFile.find(",")+1, lineFromFile.length() - lineFromFile.find(",") - 4));
    ...
    while(getline(currentPrices, lineFromFile)){
        string ticker = lineFromFile.substr(0,lineFromFile.find(","));
        double stockPrice = std::stod(lineFromFile.substr(lineFromFile.find(",")+1));
